...
class ClassCodegen private constructor(
    ...
    fun generateMethodNode(method: IrFunction): MethodNode {
        if (!method.isInline) {
            // Non-inline function nodes are only used once in `generateMethod`, so no need to cache them.
            return FunctionCodegen(method, this).generate()
        }
        return generatedInlineMethods.getOrPut(method) {
            ...
            generatedInlineMethods[method] = codegen.methodNode.copy()
            codegen.generate()
        }.copy()
    }
